...
public class NodeMove : MonoBehaviour
{
    ...
    List<Vector3> GetCurveNodes()
    {
        ...
        curveNodes.Add(parent.InverseTransformPoint(transform.position));
        ...
        for (int i = 0; i < nodes.Count - 3; i += 3)
        {
            ...
            if (i == 0)
            {
                ...
                curveNodes.Add(CalculateBezierPath(p0, p1, p2, p3, 0f));
            }

            for (int j = 0; j < CURVE_SEGMENT; j++)
            {
                float t = j / (float)CURVE_SEGMENT;
                curveNodes.Add(CalculateBezierPath(p0, p1, p2, p3, t));
            }

        }

        realLoopNode = (int)(curveNodes.Count * (loopToNode / (float)nodes.Count));
    ...
    Vector3 CalculateBezierPath(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, float t)
    {
        ...
        Vector3 result = Mathf.Pow(oneMinusT, 3f) * p0 + 3f * Mathf.Pow(oneMinusT, 2f) * t * p1 + 3f * oneMinusT * (t * t) * p2 + Mathf.Pow(t, 3f) * p3;
